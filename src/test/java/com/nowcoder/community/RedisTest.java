...
public class RedisTest {
    ...
    //事务
    //不满足四个特点
    //启用事务后，当执行一个redis命令时，并不会立刻执行，而是放入队列里，先存着，再执行一个命令，就再放入队列里，直到操作完了，
    // 提交事务的时候，会把队列里所有命令一股脑发送给redis服务器，一起执行
    //问题：事务之内的命令不会立即执行，所以在事务过程中进行一次查询，不会立即返回结果，所以不要在事务中间进行查询，要不就在事务之前查询，要不就在事务提交之后查询
    //编程式事务
    @Test
